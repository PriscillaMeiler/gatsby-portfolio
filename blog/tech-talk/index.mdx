...
Ah, outro ponto importante de lembrar é que quando rodamos o projeto localmente, além do link do projeto, também temos o link do graphQL disponível, para buscarmos e testarmos as chamadas.   
...
````
Onde o `path`, vai ser a pasta onde os arquivos MDX vão ficar e o `name` será o campo de `sourceInstanceName`, que serve para quando temos mais de uma pasta com arquivos MDX, dessa forma, conseguimos filtrar e pegar exatamente o conteúdo que queremos.
...
Arquivos MDX usam Markup pra formatação. E no caso dos posts do Blog, podemos começar o arquivo com os informações do post, por exemplo:
...
Na query, pegamos `allMdx` que fica disponível com o plugin que falamos anteriormente, o `gatsby-plugin-mdx`. E antes de fazer a chamada dos campos que queremos, ordenamos a resposta para que retorne do post mais antigo para o mais novo, com esse trecho `sort: { frontmatter: { date: DESC }}`.Dentro do node, temos o frontmatter, que são aquelas informações que colocamos antes do conteúdo, o excerpt, que é o começo do conteúdo, que foi separado automaticamente pelo plugin, e o id, que é único para cada post, criado automaticamente também.
...
É importante lembrar também, que como queremos retornar apenas um post específico, ao invés de usarmos `allMdx`, usamos `mdx` e colocamos o filtro `(id: {eq: $id})` para que a resposta seja exatamente o post que queremos, nesse caso, o porque que o ID é igual ao ID da página.